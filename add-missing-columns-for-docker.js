 ...
 * Plain JavaScript version for Docker compatibility
 */
...
console.log("Starting database column check...");
...
async function processTable(tableName, columnDefinitions) {
...
async function fixColumnNameDiscrepancy() {
  try {
    ...
    console.error('Error fixing column name discrepancy:', error);
  }
}
...
async function main() {
  try {
    ...
    for (const [tableName, columnDefinitions] of Object.entries(requiredColumns)) {
      await processTable(tableName, columnDefinitions);
    }
    
    // Fix column name discrepancy
    await fixColumnNameDiscrepancy();
